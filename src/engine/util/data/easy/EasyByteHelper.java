...
public class EasyByteHelper {
	...
	public static <T extends EasySerializable<T>> byte[] toBytes(T[] o) {
		byte[][] bytes = new byte[o.length+1][];
		bytes[0] = EasyByteHelper.toBytes(o.length);
		for (int i = 0; i < o.length; i++) {
			bytes[i+1] = EasyByteHelper.toBytes(o[i]);
		} return EasyByteHelper.mergeBytes(bytes);
	}
	
	public static byte[] toBytes(String[] s) {
		byte[][] bytes = new byte[s.length+1][];
		bytes[0] = EasyByteHelper.toBytes(s.length);
		for (int i = 0; i < s.length; i++) {
			bytes[i+1] = EasyByteHelper.toBytes(s[i]);
		} return EasyByteHelper.mergeBytes(bytes);
	}
	...
	public static <T extends EasySerializable<T>> byte[] toBytes(T object) {
		byte[] bytes = object.serialize();
		return EasyByteHelper.mergeBytes(ByteBuffer.allocate(4).putInt(bytes.length).array(), bytes);
	}

	public static byte[] toBytes(String s) {
		byte[] b = s.getBytes(StandardCharsets.UTF_8);
		return EasyByteHelper.mergeBytes(EasyByteHelper.toBytes(b.length), b);
	}
	...
	public EasyByteHelper(byte[] byteStream) {
		this.byteStream = ByteBuffer.wrap(byteStream);
	}
	
	@SuppressWarnings("unchecked")
	public <T extends EasySerializable<T>> T[] readObjArr(T prototype) {
	...
	public <T extends EasySerializable<T>> T readObj(T prototype) {
		byte[] bytes = new byte[this.byteStream.getInt()];
		this.byteStream.get(bytes);
		return prototype.deserialize(new EasyByteHelper(bytes));
	}
