...
public abstract class AbstractEntity implements Comparable<AbstractEntity> {
	...
	public AbstractEntity(Room room, PImage img, AbstractMove move, AbstractAbility[] abilities, int[][] colorLayers, int[] colorTints, boolean isTangible, boolean isBreakable) {  // Allow pre-computing color list outside class and using it here in constructor
		this.entID = AbstractEntity.id; AbstractEntity.id++; this.showXY = move.getPoint(); anim = new Animator(); this.currRoom = room; this.hash = 0;
		try {
			this.ability = new AbstractAbility[abilities.length]; this.image = img.copy(); this.moveset = move.get();
			...
			this.ability = new AbstractAbility[0]; this.isTangible = isTangible; this.isBreakable = isBreakable;
		}
	}
	...
	public ArrayList<AbstractEntity> getRoomList() {return this.currRoom.getRoom(this);} // GET ONLY WHAT IS AROUND ENTITY
	...
	// Implemented methods
	@Override
	public int compareTo(AbstractEntity e) {return Float.compare(this.getRY() + this.getH(), e.getRY() + e.getH());}
	
	// Overridden methods
	@Override
	public boolean equals(Object other) {if(other.getClass() != this.getClass()) {return false;} return this.entID == ((AbstractEntity) other).entID;}
	@Override
