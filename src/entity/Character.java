...
public abstract class Character extends Obstacle {
	...
	// Private set
	private void setOverX(float x) {if (this.checkMove(x, true)) {this.overPosit.setX(x);}}
	private void setOverY(float y) {if (this.checkMove(y, false)) {this.overPosit.setY(y);}}
	private boolean checkMove(float x, boolean isX) {return PApplet.abs((isX? this.overPosit.getX() : this.overPosit.getY()) - x) <= this.OVER_MOVE_SPEED_CAP;}
	// Private add
	private void addOverX() {this.overPosit.addX(this.getMoveX());}
	private void addOverY() {this.overPosit.addY(this.getMoveY());}
//	private void addOverXY() {this.addOverX(); this.addOverY();}
	...
	public boolean isCollide(Obstacle o) {
		...
		if (ToolKit.rectRectCollide(this.getNewX(), this.getNewY(), this.getW(), this.getH(), o.getX(), o.getY(), o.getW(), o.getH())) {
			
			if (this.getNewX() < o.getX() + o.getW() && this.overMove.getX() < 0) {
				this.setOverX(o.getX() + o.getW() + 0.0001f); this.overMove.resetX();
			}
			else if (this.getNewX() + this.getW() > o.getX() && this.overMove.getX() > 0) {
				this.setOverX(o.getX() - this.getW() - 0.0001f); this.overMove.resetX();
			}
			
			if (this.getNewY() < o.getY() + o.getH() && this.overMove.getY() < 0) {
				this.setOverY(o.getY() + o.getH() + 0.0001f); this.overMove.resetY();
			}
			else if (this.getNewY() + this.getH() > o.getY() && this.overMove.getY() > 0) {
				this.setOverY(o.getY() - this.getH() - 0.0001f); this.overMove.resetY();
			}
			if (this.overCurrDir % 2 == 0) {this.overCurrState = 1;}
//			if ((!(this.letMoveX && this.letMoveY) && this.overCurrDir % 2 == 0) || this.overMove.isZero()) {this.overCurrState = 1;}
